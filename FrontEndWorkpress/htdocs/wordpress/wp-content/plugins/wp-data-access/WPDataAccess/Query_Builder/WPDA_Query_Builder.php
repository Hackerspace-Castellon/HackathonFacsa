	...
	class WPDA_Query_Builder {
		...
		protected function html() {
			...
			<div class="wrap">
				<span style="float:right">
					<label style="font-weight:normal"><input type="checkbox" id="wpda_sql_hints" <?php echo 'on' === $this->sql_hints ? 'checked' : ''; ?>> Enable SQL hints</label>
				</span>
		...
		protected function js() {
			?>
				<script type="application/javascript">
					...
					function getColumns(schema, table, activeIndex, callback) {
						var url = location.pathname + '?action=wpda_get_table_widget_info';
						var data = {
						...
						jQuery.post(
							...
							data,
							function (data) {
								callback(schema, table, activeIndex, JSON.parse(data));
							}
						);
					}
		...
		public function get_db_hints() {
			...
			if (
				current_user_can( 'manage_options' ) &&
				isset(
					$_REQUEST['wpda_wpnonce'],
					$_REQUEST['wpda_schemaname']
				)
			) {
				$wpda_wpnonce    = sanitize_text_field( wp_unslash( $_REQUEST['wpda_wpnonce'] ) ); // input var okay.
				$wpda_schemaname = sanitize_text_field( wp_unslash( $_REQUEST['wpda_schemaname'] ) ); // input var okay.
		...
		public function set_db_hints() {
			...
			if (
				...
				isset(
					$_REQUEST['wpda_wpnonce'],
					$_REQUEST['wpda_sqlhints']
				)
			) {
				$wpda_wpnonce  = sanitize_text_field( wp_unslash( $_REQUEST['wpda_wpnonce'] ) ); // input var okay.
				$wpda_sqlhints = sanitize_text_field( wp_unslash( $_REQUEST['wpda_sqlhints'] ) ); // input var okay.
